...
def first_run(self):
    data = ('John', 'Doe', 'John Doe','123456','jdoe@email.com')
    conn = sqlite3.connect('studentTracking.db')
    with conn:
        cur = conn.cursor()
        cur,count = count_records(cur)
        if count < 1:
            cur.execute("""INSERT INTO tbl_student (col_fname,col_lname,col_fullname,col_course,col_email) VALUES (?,?,?,?,?)""", (data))
            conn.commit()
    conn.close()
...
def onSelect(self,event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_course,col_email FROM tbl_student WHERE col_fullname = (?)""", [value])
...
def addToList(self):
    ...
    var_courseNumber = self.txt_courseNumber.get().strip()
    var_email = self.txt_email.get().strip()
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_courseNumber) > 0) and(len(var_email) > 0): # enforce the user to provide both names
        conn = sqlite3.connect('studentTracking.db')
        with conn:
            cursor = conn.cursor()
            ...
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                ...
                cursor.execute("""INSERT INTO tbl_student (col_fname,col_lname,col_fullname,col_course,col_email) VALUES (?,?,?,?,?)""")
...
def onUpdate(self):
    ...
    var_course = self.txt_course.get().strip() # normalize the data to maintain database integrity
    var_email = self.txt_email.get().strip()
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            if count == 0 or count2 == 0: # if proposed changes are not already in the database, then proceed
                response = messagebox.askokcancel("Update Request","The following changes ({}) and ({}) will be implemented for ({}). \n\nProceed with the update request?".format(var_course,var_email,var_value))
                print(response)
                if response:
                    #conn = sqlite3.connect('studentTracking.db')
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_student SET col_course = '{0}',col_email = '{1}' WHERE col_fullname = '{2}'""".format(var_course,var_email,var_value))
                        onClear(self)
                        conn.commit()
                else:
                    messagebox.showinfo("Cancel request","No changes have been made to ({}).".format(var_value))
            else:
                messagebox.showinfo("No changes detected","Both ({}) and ({}) \nalready exist in the database for this name. \n\nYour update request has been cancelled.".format(var_course, var_email))
            onClear(self)
        conn.close()
    else:
        messagebox.showerror("Missing information","Please select a name from the list. \nThen edit the course number or email information.")
    onClear(self)
